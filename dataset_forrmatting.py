...
def convert(transcripts):
    ...
    with Pool(os.cpu_count()) as p:
        p.map(functools.partial(convert_internal, transcripts), dirs)

def convert_internal(transcripts, di):
    file_dirs = os.listdir(os.path.join(origin_path, di, origin2file_dir))
    if not os.path.exists(dst_path):
        os.mkdir(dst_path)
    jpv_dir = os.path.join(dst_path, di[-3:] + '_jpv')
    if not os.path.exists(jpv_dir):
        os.mkdir(jpv_dir)
    jpv_text = os.path.join(jpv_dir, 'text')
    if not os.path.exists(jpv_text):
        os.mkdir(jpv_text)
    jpv_wav = os.path.join(jpv_dir, 'wav')
    if not os.path.exists(jpv_wav):
        os.mkdir(jpv_wav)
    conv_dir1 = os.path.join(dst_path, str(int(di[-3]) + 1) + di[-2:] + '_jpv_conv10')
    if not os.path.exists(conv_dir1):
        os.mkdir(conv_dir1)
    conv_text1 = os.path.join(conv_dir1, 'text')
    if not os.path.exists(conv_text1):
        os.mkdir(conv_text1)
    conv_wav1 = os.path.join(conv_dir1, 'wav')
    if not os.path.exists(conv_wav1):
        os.mkdir(conv_wav1)
    conv_dir2 = os.path.join(dst_path, str(int(di[-3]) + 2) + di[-2:] + '_jpv_conv20')
    if not os.path.exists(conv_dir2):
        os.mkdir(conv_dir2)
    conv_text2 = os.path.join(conv_dir2, 'text')
    if not os.path.exists(conv_text2):
        os.mkdir(conv_text2)
    conv_wav2 = os.path.join(conv_dir2, 'wav')
    if not os.path.exists(conv_wav2):
        os.mkdir(conv_wav2)
    conv_dir3 = os.path.join(dst_path, str(int(di[-3]) + 3) + di[-2:] + '_jpv_conv30')
    if not os.path.exists(conv_dir3):
        os.mkdir(conv_dir3)
    conv_text3 = os.path.join(conv_dir3, 'text')
    if not os.path.exists(conv_text3):
        os.mkdir(conv_text3)
    conv_wav3 = os.path.join(conv_dir3, 'wav')
    ...
    for file in file_dirs:
        ...
        with open(os.path.join(conv_text3, dst_name + ".txt"), mode='w') as f:
            f.write(transcripts[file_name])
        shutil.copy(os.path.join(origin_path, di, origin2file_dir, file), os.path.join(jpv_wav, dst_name + ".wav"))
        with wave.open(os.path.join(origin_path, di, origin2file_dir, file), 'r') as f:
...
def create_datasets():
    for speaker in range(1, 101):
        for type in range(1, 4):
            ...
            jpv_dir = os.path.join(dst_path, "{:0>3d}".format(speaker) + '_jpv')
            conv_dir = os.path.join(dst_path, str(speaker + type * 100) + '_jpv_conv' + str(type * 10))
            ...
            for lab, wav in zip(jpv_text, jpv_wav):
                with open(os.path.join(jpv_dir, 'text', lab), 'r', encoding="shift_jis") as f:
                    mozi = f.read().split("\n")
                test = create_dataset_jtalk.mozi2phone(str(mozi))
                wav = os.path.abspath(os.path.join(jpv_dir, 'wav', wav))
                if counter % 10 != 0:
            ...
            for lab, wav in zip(conv_text, conv_wav):
                with open(os.path.join(conv_dir, 'text', lab), 'r', encoding="shift_jis") as f:
                    mozi = f.read().split("\n")
                test = create_dataset_jtalk.mozi2phone(str(mozi))
                wav = os.path.abspath(os.path.join(conv_dir, 'wav', wav))
                if counter % 10 != 0:
